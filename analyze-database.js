...
async function getDatabaseList(connection) {
   try {
      ...
      return rows.map((row) => row.Database).filter((name) => name.startsWith('school_erp_'));
   } catch (error) {
...
async function checkFrontendGaps(schemas) {
   ...
   const existingRoutes = fs.existsSync(routesDir) ? fs.readdirSync(routesDir).map((f) => path.basename(f, '.js')) : [];

   // Get existing view directories
   const existingViews = fs.existsSync(viewsDir) ? fs.readdirSync(viewsDir) : [];
