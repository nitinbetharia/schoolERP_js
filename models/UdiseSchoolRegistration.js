...
const udiseSchoolRegistrationValidationSchemas = {
  ...
  createRegistration: Joi.object({
    ...
    school_category: Joi.string()
      ...
      .messages({
        "any.only":
          "School category must be primary, upper_primary, secondary, or higher_secondary",
    ...
    school_management: Joi.string()
      ...
      .messages({
        "any.only":
          "School management must be government, government_aided, private_unaided, or other",
        "any.required": "School management is required",
      }),
    school_type: Joi.string()
      .valid("boys", "girls", "co_educational")
      .required()
      .messages({
        "any.only": "School type must be boys, girls, or co_educational",
        "any.required": "School type is required",
      }),
    recognition_status: Joi.string()
      .valid("recognized", "unrecognized", "partially_recognized")
      .required()
      .messages({
        "any.only":
          "Recognition status must be recognized, unrecognized, or partially_recognized",
  ...
  queryRegistrations: Joi.object({
    ...
    sortBy: Joi.string()
      .valid("id", "udise_code", "registration_date", "school_name_english")
      .default("id"),
  ...
  const UdiseSchoolRegistration = sequelize.define(
    "UdiseSchoolRegistration",
    {
      ...
      affiliation_board: {
        ...
        allowNull: true,
        comment:
          "Educational board affiliation (CBSE, NCERT, State Board, etc.)",
      },
      affiliation_number: {
        type: DataTypes.STRING(50),
        allowNull: true,
