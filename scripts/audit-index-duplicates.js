...
function findJSFiles(dir, files = []) {
  ...
  for (const item of items) {
    ...
    if (
      ...
      item.endsWith(".js") &&
      (dir.includes("/models") || dir.includes("\\models"))
    ) {
      files.push(fullPath);
    }
  }
...
function main() {
  ...
  if (issues.length > 0) {
    ...
    issues.forEach((issue) => {
      console.log(`\n   📁 ${issue.file}`);
      console.log(
        `      🔑 Field-level unique: ${issue.uniqueFields.join(", ")}`,
      );
      console.log(
        `      📊 Explicit unique indexes: ${issue.explicitUniqueIndexes.join(", ")}`,
      );
  ...
  console.log(
    `   the MySQL 64-key limit issue (field unique + explicit unique index).`,
  );
}
