...
class DocumentationVerifier {
  constructor() {
    this.errors = [];
    this.warnings = [];
    this.mainDocPath = path.join(
      __dirname,
      "../docs/COMPLETE_DOCUMENTATION.md",
    );
    this.readmePath = path.join(__dirname, "../README.md");
    this.copilotInstructionsPath = path.join(
      __dirname,
      "../docs/archived/COPILOT_INSTRUCTIONS.md",
    );
  ...
  async checkDatabaseNameConsistency() {
    ...
    const patterns = [
      {
        regex: /school_erp_master/gi,
        violation: "Should be school_erp_system",
      },
    ...
    for (const file of files) {
      ...
      if (content.match(patterns[0].regex)) {
        this.errors.push(
          `${file}: Contains 'school_erp_master' instead of 'school_erp_system'`,
        );
      }
    }
  }
      ...
      for (const pattern of versionPatterns) {
        ...
        for (const match of matches) {
          ...
          if (version && !["18+", "18", "18.0", "18.0.0"].includes(version)) {
            this.warnings.push(
              `${file}: Node.js version '${version}' should be '18+'`,
            );
          }
        }
      }
  ...
  async checkCredentialsConsistency() {
    ...
    const credentialPatterns = [
      {
        regex:
          /(?:email|Email|EMAIL).*(?:nitin@gmail\.com|admin@system\.local)/gi,
        field: "email",
      },
      {
    ...
    for (const file of files) {
      ...
      if (
        content.includes("nitin@gmail.com") ||
        content.includes("nitin@123")
      ) {
        this.errors.push(
          `${file}: Contains old credentials (nitin@gmail.com/nitin@123) instead of admin@system.local/admin123`,
        );
      }
    }
  }
      ...
      for (const pattern of mysqlPatterns) {
        if (content.match(pattern.regex)) {
          ...
          if (!file.includes("CRITICAL_MYSQL_SHELL_INSTRUCTION.md")) {
            this.warnings.push(
              `${file}: Contains 'mysql' command - ${pattern.violation}`,
            );
          }
        }
      }
  ...
  async checkTechStackConsistency() {
    ...
    const techChecks = [
      {
        tech: "Express",
        expected: ["Express.js", "Express"],
        variations: ["express.js", "ExpressJS"],
      },
      {
        tech: "MySQL",
        expected: ["MySQL"],
        variations: ["MySql", "mysql", "My SQL"],
      },
      { tech: "Sequelize", expected: ["Sequelize"], variations: ["sequelize"] },
      { tech: "EJS", expected: ["EJS"], variations: ["ejs", "Ejs"] },
      {
        tech: "Tailwind",
        expected: ["Tailwind CSS", "Tailwind"],
        variations: ["tailwind", "TailwindCSS"],
      },
    ];
      ...
      for (const check of techChecks) {
        for (const variation of check.variations) {
          ...
          if (content.match(regex)) {
            this.warnings.push(
              `${file}: Uses '${variation}' instead of preferred '${check.expected[0]}'`,
            );
          }
        }
      }
  ...
  async checkConfigurationPatterns() {
    ...
    const violationPatterns = [
      ...
      },
      {
        ...
        violation:
          "Database names should come from JSON config, not environment",
      },
    ];
...
function suggestFixes() {
  ...
  console.log("-".repeat(50));
  console.log(
    '1. Database names: Replace "school_erp_master" with "school_erp_system"',
  );
