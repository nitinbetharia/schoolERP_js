...
async function setupSystemDatabase() {
  ...
  try {
    ...
    const initialConfig = [
      { config_key: 'system_version', config_value: '1.0.0', data_type: 'STRING', description: 'System version' },
      { config_key: 'maintenance_mode', config_value: 'false', data_type: 'BOOLEAN', description: 'System maintenance mode' },
      { config_key: 'max_trusts', config_value: '100', data_type: 'NUMBER', description: 'Maximum number of trusts allowed' },
      { config_key: 'default_theme', config_value: 'default', data_type: 'STRING', description: 'Default theme for new trusts' }
    ];
    ...
    for (const configItem of initialConfig) {
      try {
        await systemSequelize.query(
          ...
          {
            replacements: [configItem.config_key, configItem.config_value, configItem.data_type, configItem.description]
          }
        );
      } catch (error) {
