...
const { ErrorFactory, formatSuccessResponse, validators, commonSchemas } = require('../../../utils/errors');
...
function createUserController() {
   ...
   async function listUsers(req, res, next) {
      try {
         ...
         res.json(
            formatSuccessResponse(result.users, 'Users retrieved successfully', { pagination: result.pagination })
         );
      } catch (error) {
         next(error);
      }
   ...
   async function changePassword(req, res, next) {
      try {
         ...
         await userService.updateUser(tenantCode, userId, { password: newPassword });
   ...
   async function getUserRoles(req, res, next) {
      try {
         res.json(formatSuccessResponse(userService.defaultRoles, 'User roles retrieved successfully'));
      } catch (error) {
         next(error);
      }
   }
