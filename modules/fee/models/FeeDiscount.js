...
function defineFeeDiscount(sequelize) {
  ...
  FeeDiscount.prototype.isApplicationOpen = function () {
    ...
    if (
      ...
      today < new Date(this.application_start_date)
    ) {
      return false;
    }
    ...
    if (
      ...
      today > new Date(this.application_end_date)
    ) {
      return false;
    }
  ...
  FeeDiscount.prototype.isApplicableToFeeCategory = function (feeCategory) {
    if (
      !this.applicable_fee_categories ||
      this.applicable_fee_categories.length === 0
    ) {
  ...
  FeeDiscount.prototype.isApplicableToAcademicYear = function (academicYear) {
    if (
      !this.applicable_academic_years ||
      this.applicable_academic_years.length === 0
    ) {
  ...
  FeeDiscount.prototype.checkEligibility = function (student, studentFee) {
    try {
      // Basic validations
      if (!this.isValid())
        return { eligible: false, reason: "Discount is not valid" };
      ...
      if (!this.isApplicableToAcademicYear(studentFee.academic_year)) {
        return {
          eligible: false,
          reason: "Not applicable to current academic year",
        };
      }
      ...
      if (this.eligibility_criteria) {
        ...
        if (
          ...
          student.academic_percentage < criteria.min_percentage
        ) {
          return {
            eligible: false,
            reason: "Does not meet minimum academic requirement",
          };
        }
        ...
        if (
          ...
          student.family_income > criteria.max_family_income
        ) {
          return {
            eligible: false,
            reason: "Family income exceeds maximum limit",
          };
        }

        // Check special categories if specified
        if (
          criteria.required_category &&
          !criteria.required_category.includes(student.category)
        ) {
          return {
            eligible: false,
            reason: "Does not belong to required category",
          };
        }
      }
  ...
  FeeDiscount.prototype.updateBudgetUtilization = async function (amount) {
    try {
      this.budget_utilized =
        parseFloat(this.budget_utilized) + parseFloat(amount);

      if (
        this.budget_allocated &&
        this.budget_utilized > this.budget_allocated
      ) {
  ...
  FeeDiscount.getActiveDiscounts = async function (
    schoolId,
    feeCategory = null,
    className = null,
  ) {
    try {
      ...
      let filteredDiscounts = discounts.filter((discount) =>
        discount.isValid(),
      );

      if (feeCategory) {
        filteredDiscounts = filteredDiscounts.filter((discount) =>
          discount.isApplicableToFeeCategory(feeCategory),
        );
      }

      if (className) {
        filteredDiscounts = filteredDiscounts.filter((discount) =>
          discount.isApplicableToClass(className),
        );
      }
  ...
  FeeDiscount.calculateBestDiscountCombination = function (
    discounts,
    feeAmount,
  ) {
    try {
      ...
      const cumulativeDiscounts = discounts.filter(
        (d) => d.cumulative_with_others,
      );
      const nonCumulativeDiscounts = discounts.filter(
        (d) => !d.cumulative_with_others,
      );
      ...
      if (cumulativeDiscounts.length > 0) {
        ...
        const sortedCumulative = cumulativeDiscounts.sort(
