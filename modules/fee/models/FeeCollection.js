...
function defineFeeCollection(sequelize) {
   const FeeCollection = sequelize.define(
      'FeeCollection',
      {
         ...
         payment_method: {
            type: DataTypes.ENUM,
            values: ['CASH', 'CHEQUE', 'DD', 'BANK_TRANSFER', 'ONLINE', 'CARD', 'UPI', 'WALLET', 'OTHER'],
            allowNull: false,
            defaultValue: 'CASH',
         ...
         payment_status: {
            type: DataTypes.ENUM,
            values: ['SUCCESS', 'PENDING', 'FAILED', 'CANCELLED', 'REFUNDED', 'BOUNCED'],
            allowNull: false,
   ...
   FeeCollection.prototype.canBeCancelled = function () {
      return ['SUCCESS', 'PENDING'].includes(this.payment_status) && !this.reconciled;
   };

   FeeCollection.prototype.canBeRefunded = function () {
      return this.payment_status === 'SUCCESS' && this.refund_amount < this.payment_amount;
   };
   ...
   FeeCollection.prototype.processRefund = function (refundAmount, refundReason, refundReference) {
   ...
   FeeCollection.generateReceiptNumber = async function (schoolId, academicYear) {
      try {
   ...
   FeeCollection.collectPayment = async function (paymentData, collectedBy) {
      ...
      try {
         ...
         const receiptNumber = await this.generateReceiptNumber(paymentData.school_id, paymentData.academic_year);
         ...
         const studentFee = await this.sequelize.models.StudentFee.findByPk(paymentData.student_fee_id, {
            transaction,
         });
   ...
   FeeCollection.getDailyCollection = async function (schoolId, date) {
      try {
         ...
         const summary = {
            ...
            total_transactions: result.reduce((sum, item) => sum + parseInt(item.transaction_count), 0),
            total_amount: result.reduce((sum, item) => sum + parseFloat(item.total_amount), 0),
         };
   ...
   FeeCollection.getCollectionStats = async function (schoolId, startDate, endDate) {
      try {
