...
const feeStructureValidationSchemas = {
  ...
  createFeeStructure: Joi.object({
    ...
    fee_type: Joi.string()
      .valid("MANDATORY", "OPTIONAL", "CONDITIONAL")
      .default("MANDATORY")
      .messages({
    ...
    frequency: Joi.string()
      ...
      .messages({
        "any.only":
          "Frequency must be YEARLY, HALF_YEARLY, QUARTERLY, MONTHLY, or ONE_TIME",
      }),
    due_date: Joi.date().optional().allow(null),
    due_month: Joi.number()
      .integer()
      .min(1)
      .max(12)
      .optional()
    ...
    late_fee_percentage: Joi.number()
      .min(0)
      .max(100)
      .precision(2)
      .optional()
  ...
  queryFeeStructures: Joi.object({
    ...
    sortBy: Joi.string()
      .valid("id", "fee_category", "amount", "effective_from", "created_at")
      .default("id"),
...
function defineFeeStructure(sequelize) {
  ...
  FeeStructure.prototype.calculateLateFee = function () {
    ...
    if (this.late_fee_amount) {
      ...
      lateFee =
        (parseFloat(this.amount) * parseFloat(this.late_fee_percentage)) / 100;
    }
  ...
  FeeStructure.calculateTotalFeeForClass = async function (
    classId,
    academicYear,
  ) {
    try {
  ...
  FeeStructure.getEffectiveStructure = async function (
    schoolId,
    classId,
    academicYear,
    date = new Date(),
