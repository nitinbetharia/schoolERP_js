...
function defineStudentFeeDiscount(sequelize) {
  ...
  StudentFeeDiscount.prototype.isExpired = function () {
    return (
      this.application_status === "EXPIRED" ||
      (this.effective_until && new Date() > new Date(this.effective_until))
    );
  };
  ...
  StudentFeeDiscount.prototype.canBeApproved = function () {
    return (
      ...
      (this.verification_status === "VERIFIED" ||
        this.verification_status === "NOT_REQUIRED")
    );
  };
  ...
  StudentFeeDiscount.prototype.approve = async function (
    approvedBy,
    remarks = null,
  ) {
    try {
  ...
  StudentFeeDiscount.prototype.addToAuditTrail = function (
    action,
    userId,
    notes = null,
  ) {
  ...
  StudentFeeDiscount.prototype.renew = async function (
    renewedBy,
    newExpiryDate,
  ) {
    try {
  ...
  StudentFeeDiscount.applyDiscount = async function (
    studentFeeId,
    discountId,
    appliedBy,
    options = {},
    ...
    try {
      ...
      const discount = await this.sequelize.models.FeeDiscount.findByPk(
        discountId,
        { transaction },
      );
      ...
      const studentFeeDiscount = await this.create(
        {
          ...
          application_status: discount.requires_approval
            ? "PENDING"
            : "APPROVED",
          verification_status: discount.document_requirements
            ? "PENDING"
            : "NOT_REQUIRED",
      ...
      // Update audit trail
      studentFeeDiscount.addToAuditTrail(
        "CREATED",
        appliedBy,
        "Discount application created",
      );
  ...
  StudentFeeDiscount.getPendingApprovals = async function (
    schoolId,
    userId = null,
  ) {
    try {
  ...
  StudentFeeDiscount.getDiscountStatistics = async function (
    schoolId,
    academicYear,
  ) {
    try {
