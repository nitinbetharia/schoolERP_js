...
function defineFeeInstallment(sequelize) {
  ...
  FeeInstallment.prototype.isOverdue = function () {
    return (
      ...
      (this.isOutstanding() && this.getDaysOverdue() > this.grace_period_days)
    );
  };
  ...
  FeeInstallment.prototype.calculateLateFee = function () {
    ...
    const daysOverdue = Math.max(
      0,
      this.getDaysOverdue() - this.grace_period_days,
    );
  ...
  FeeInstallment.createInstallments = async function (
    studentFeeId,
    feeStructure,
    studentFee,
  ) {
    ...
    try {
      ...
      for (let i = 0; i < installmentPlan.length; i++) {
        ...
        const installmentAmount =
          (studentFee.total_amount * plan.percentage) / 100;
  ...
  FeeInstallment.getOverdueInstallments = async function (
    schoolId,
    graceDays = 0,
  ) {
    try {
