...
const { ErrorFactory, formatErrorResponse, getErrorStatusCode } = require('../../../utils/errors');
...
function createFeeController() {
   ...
   async function createFeeStructure(req, res) {
      try {
         ...
         if (!tenantCode) {
            throw ErrorFactory.createError('ValidationError', 'Tenant context required');
         }

         const feeStructure = await feeService.createFeeStructure(tenantCode, feeStructureData, userId);
   ...
   async function getFeeStructuresBySchool(req, res) {
      try {
         ...
         if (!tenantCode || !schoolId) {
            throw ErrorFactory.createError('ValidationError', 'Tenant context and school ID required');
         }

         const feeStructures = await feeService.getFeeStructuresBySchool(tenantCode, parseInt(schoolId), {
            academic_year,
            fee_type,
         });
   ...
   async function createStudentFee(req, res) {
      try {
         ...
         const studentFee = await feeService.createStudentFee(tenantCode, studentFeeData, userId);
   ...
   async function getStudentFees(req, res) {
      try {
         ...
         if (!tenantCode || !studentId) {
            throw ErrorFactory.createError('ValidationError', 'Tenant context and student ID required');
         }

         const studentFees = await feeService.getStudentFees(tenantCode, parseInt(studentId), {
            academic_year,
            status,
         });
   ...
   async function processFeePayment(req, res) {
      try {
         ...
         const paymentResult = await feeService.processFeePayment(tenantCode, paymentData, userId);
   ...
   async function getFeeCollections(req, res) {
      try {
         const { tenantCode } = req.session;
         const { student_id, school_id, from_date, to_date, payment_method, limit = 50, offset = 0 } = req.query;
         ...
         const collections = await feeService.getFeeCollections(tenantCode, filters, {
   ...
   async function createFeeDiscount(req, res) {
      try {
         ...
         const discount = await feeService.createFeeDiscount(tenantCode, discountData, userId);
   ...
   async function applyStudentDiscount(req, res) {
      try {
         ...
         const studentDiscount = await feeService.applyStudentDiscount(tenantCode, discountApplicationData, userId);
   ...
   async function getFeeReports(req, res) {
      try {
         const { tenantCode } = req.session;
         const { report_type, school_id, academic_year, from_date, to_date } = req.query;

         if (!tenantCode || !report_type || !school_id) {
            throw ErrorFactory.createError('ValidationError', 'Tenant context, report type, and school ID required');
         }
         ...
         switch (report_type) {
            case 'collection_summary':
               reportData = await feeService.generateCollectionSummaryReport(tenantCode, filters);
               break;
            case 'outstanding_fees':
               reportData = await feeService.generateOutstandingFeesReport(tenantCode, filters);
               break;
            case 'defaulter_list':
               reportData = await feeService.generateDefaulterListReport(tenantCode, filters);
               break;
            case 'discount_summary':
               reportData = await feeService.generateDiscountSummaryReport(tenantCode, filters);
               break;
            default:
               throw ErrorFactory.createError('ValidationError', 'Invalid report type');
         }
   ...
   async function bulkFeeOperations(req, res) {
      try {
         ...
         if (!tenantCode || !operation || !data) {
            throw ErrorFactory.createError('ValidationError', 'Tenant context, operation, and data required');
         }
         ...
         switch (operation) {
            case 'create_student_fees':
               result = await feeService.bulkCreateStudentFees(tenantCode, data, userId);
               break;
            case 'apply_discounts':
               result = await feeService.bulkApplyDiscounts(tenantCode, data, userId);
               break;
            case 'generate_installments':
               result = await feeService.bulkGenerateInstallments(tenantCode, data, userId);
               break;
            default:
               throw ErrorFactory.createError('ValidationError', 'Invalid bulk operation');
         }
