...
class FeeService {
   ...
   async createFeeStructure(feeStructureData, createdBy) {
      try {
         ...
         if (!feeStructureData.fee_name || !feeStructureData.fee_category || !feeStructureData.amount) {
            throw ErrorFactory.badRequest('Fee name, category, and amount are required');
         }
         ...
         if (existingFee) {
            throw ErrorFactory.conflict('Fee structure already exists for this class and category');
         }
    ...
    */
   async assignFeeToStudent(studentId, feeStructureId, academicYear, assignedBy) {
      try {
         ...
         if (feeStructure.installment_plan && feeStructure.installment_plan.length > 0) {
            await this.FeeInstallment.createInstallments(studentFee.id, feeStructure, studentFee);
         }
   ...
   async processPayment(paymentData, collectedBy) {
      try {
         ...
         const studentFee = await this.StudentFee.findByPk(paymentData.student_fee_id);
         if (!studentFee) {
         ...
         const collection = await this.FeeCollection.collectPayment(paymentData, collectedBy);
   ...
   async applyAutoDiscounts(studentFeeId, appliedBy) {
      try {
         ...
         const discounts = await this.FeeDiscount.getAutoApplyDiscounts(studentFee.school_id);

         for (const discount of discounts) {
            try {
               // Check eligibility
               const eligibility = discount.checkEligibility(studentFee.student, studentFee);
               if (eligibility.eligible) {
                  await this.StudentFeeDiscount.applyDiscount(studentFeeId, discount.id, appliedBy, {
                     auto_applied: true,
                  });
               }
            } catch (error) {
    ...
    */
   async applyDiscountToStudentFee(studentFeeId, discountId, appliedBy, options = {}) {
      try {
         return await this.StudentFeeDiscount.applyDiscount(studentFeeId, discountId, appliedBy, options);
      } catch (error) {
   ...
   async getOverdueInstallments(schoolId, graceDays = 0) {
      try {
         return await this.FeeInstallment.getOverdueInstallments(schoolId, graceDays);
      } catch (error) {
   ...
   async getFeeCollectionSummary(schoolId, startDate, endDate) {
      try {
         const stats = await this.FeeCollection.getCollectionStats(schoolId, startDate, endDate);
         return stats;
      } catch (error) {
   ...
   async getOutstandingFeesReport(schoolId, academicYear) {
      try {
            ...
            include: [
               {
                  model: this.Student,
                  as: 'student',
                  attributes: ['id', 'admission_number', 'first_name', 'last_name', 'class_name'],
               },
               {
                  model: this.FeeStructure,
                  as: 'feeStructure',
         ...
         const summary = {
            school_id: schoolId,
            academic_year: academicYear,
            total_outstanding_amount: outstandingFees.reduce((sum, fee) => sum + parseFloat(fee.due_amount), 0),
            total_outstanding_fees: outstandingFees.length,
            overdue_fees: outstandingFees.filter((fee) => fee.payment_status === 'OVERDUE').length,
   ...
   async getDiscountUtilizationReport(schoolId, academicYear) {
      try {
         const discountStats = await this.StudentFeeDiscount.getDiscountStatistics(schoolId, academicYear);

         const discountBreakdown = await this.StudentFeeDiscount.findAll({
            attributes: [
               [this.models.sequelize.fn('COUNT', this.models.sequelize.col('StudentFeeDiscount.id')), 'usage_count'],
               [
                  this.models.sequelize.fn('SUM', this.models.sequelize.col('StudentFeeDiscount.discount_amount')),
                  'total_discount',
               ],
            ],
            where: {
            ...
            include: [
               {
                  model: this.FeeDiscount,
                  as: 'feeDiscount',
                  attributes: ['id', 'discount_name', 'discount_type', 'discount_code'],
               },
            ],
   ...
   async bulkAssignFees(studentIds, feeStructureId, academicYear, assignedBy) {
      try {
         ...
         for (const studentId of studentIds) {
            try {
               const studentFee = await this.assignFeeToStudent(studentId, feeStructureId, academicYear, assignedBy);
               results.push({
                  student_id: studentId,
                  success: true,
                  student_fee_id: studentFee.id,
