...
function createUdiseService() {
   ...
   async function getModels(tenantCode) {
      try {
         ...
         throw ErrorFactory.createDatabaseError('Failed to get UDISE models', error);
      }
   }
   ...
   const registrationService = {
      ...
      async createRegistration(tenantCode, schoolId, registrationData, createdBy) {
         try {
            logSystem(`Creating UDISE registration for school: ${schoolId}`, { tenantCode });
            ...
            if (existingRegistration) {
               throw ErrorFactory.createValidationError('UDISE registration already exists for this school');
            }
            ...
            for (const field of requiredFields) {
               if (!registrationData[field]) {
                  throw ErrorFactory.createValidationError(`${field} is required for UDISE registration`);
               }
            }
            ...
            logError(error, { context: 'UdiseService.createRegistration', tenantCode, schoolId });
      ...
      async updateRegistration(tenantCode, registrationId, updateData, updatedBy) {
         try {
            ...
            if (!registration) {
               throw ErrorFactory.createNotFoundError('UDISE registration not found');
            }

            // Prevent updating certain fields if already approved
            if (registration.registration_status === 'approved' && updateData.udise_code) {
               throw ErrorFactory.createValidationError('Cannot modify UDISE code for approved registration');
            }
            ...
            logSystem(`UDISE registration updated: ${registrationId}`, { tenantCode });
            return registration;
         } catch (error) {
            logError(error, { context: 'UdiseService.updateRegistration', tenantCode, registrationId });
            throw error;
         }
      },
      ...
      async submitRegistration(tenantCode, registrationId, submittedBy) {
         try {
            ...
            if (registration.registration_status !== 'draft') {
               throw ErrorFactory.createValidationError('Only draft registrations can be submitted');
            }
            ...
            if (!validationResult.isValid) {
               throw ErrorFactory.createValidationError('Registration validation failed', validationResult.errors);
            }
            ...
            logSystem(`UDISE registration submitted: ${registrationId}`, { tenantCode });
            return registration;
         } catch (error) {
            logError(error, { context: 'UdiseService.submitRegistration', tenantCode, registrationId });
            throw error;
         }
      },
      ...
      async getRegistrations(tenantCode, filters = {}) {
         try {
            ...
            if (filters.registration_status) whereClause.registration_status = filters.registration_status;
            if (filters.state_code) whereClause.state_code = filters.state_code;
            if (filters.district_code) whereClause.district_code = filters.district_code;
            if (filters.academic_year) whereClause.academic_year = filters.academic_year;
            ...
            logError(error, { context: 'UdiseService.getRegistrations', tenantCode });
            throw error;
         }
      },
      ...
      async validateRegistration(tenantCode, registrationId) {
         try {
            ...
            logError(error, { context: 'UdiseService.validateRegistration', tenantCode, registrationId });
            throw error;
         }
      },
   };
   ...
   const censusService = {
      ...
      async createCensusData(tenantCode, udiseRegistrationId, censusData, createdBy) {
         try {
            ...
            if (existingCensus) {
               throw ErrorFactory.createValidationError('Census data already exists for this period');
            }
            ...
            logError(error, { context: 'UdiseService.createCensusData', tenantCode });
            throw error;
         }
      },
      ...
      async calculateEnrollmentStats(tenantCode, censusId) {
         try {
            ...
            logError(error, { context: 'UdiseService.calculateEnrollmentStats', tenantCode });
            throw error;
         }
      },
   };
   ...
   const complianceService = {
      ...
      async createComplianceRecord(tenantCode, udiseRegistrationId, complianceData, createdBy) {
         try {
            ...
            logSystem(`Compliance record created: ${compliance.id}`, { tenantCode });
            return compliance;
         } catch (error) {
            logError(error, { context: 'UdiseService.createComplianceRecord', tenantCode });
            throw error;
         }
      },
   ...
   async function logIntegration(tenantCode, schoolId, logData) {
      try {
         ...
         logError(error, { context: 'UdiseService.logIntegration', tenantCode, schoolId });
   ...
   async function generateReports(tenantCode, reportType, filters = {}) {
      try {
         ...
         logError(error, { context: 'UdiseService.generateReports', tenantCode, reportType });
         throw error;
      }
   }
