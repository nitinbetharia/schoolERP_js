...
class AttendanceController {
  ...
  async markStudentAttendance(req, res, next) {
    try {
      ...
      const attendance = await this.attendanceService.markStudentAttendance(
        attendanceData,
        req.user.id,
      );
  ...
  async markBulkStudentAttendance(req, res, next) {
    try {
      ...
      if (!Array.isArray(attendance_list) || attendance_list.length === 0) {
        throw ErrorFactory.createError(
          "BAD_REQUEST",
          "attendance_list must be a non-empty array",
        );
      }
      ...
      const results = await this.attendanceService.markBulkStudentAttendance(
        attendance_list,
        req.user.id,
      );
  ...
  async updateStudentAttendance(req, res, next) {
    try {
      ...
      const attendance = await this.attendanceService.updateStudentAttendance(
        parseInt(id),
        updateData,
        req.user.id,
      );
    ...
    try {
      const filters = {
        ...
        academic_year: req.query.academic_year
          ? parseInt(req.query.academic_year)
          : undefined,
        is_holiday:
          req.query.is_holiday !== undefined
            ? req.query.is_holiday === "true"
            : undefined,
      };
      ...
      const attendance =
        await this.attendanceService.getStudentAttendance(filters);
  ...
  async getStudentAttendanceStats(req, res, next) {
    try {
      ...
      const stats =
        await this.attendanceService.getStudentAttendanceStats(filters);
  ...
  async markTeacherAttendance(req, res, next) {
    try {
      ...
      if (status === "LEAVE" && !leave_type) {
        throw ErrorFactory.createError(
          "BAD_REQUEST",
          "leave_type is required when status is LEAVE",
        );
      }
      ...
      const attendance = await this.attendanceService.markTeacherAttendance(
        attendanceData,
        req.user.id,
      );
  ...
  async updateTeacherCheckOut(req, res, next) {
    try {
      ...
      const formattedTime =
        check_out_time.length === 5 ? `${check_out_time}:00` : check_out_time;
  ...
  async getTeacherAttendance(req, res, next) {
    try {
      const filters = {
        ...
        is_holiday:
          req.query.is_holiday !== undefined
            ...
            : undefined,
        is_substitute:
          req.query.is_substitute !== undefined
            ? req.query.is_substitute === "true"
            : undefined,
      };
      ...
      const attendance =
        await this.attendanceService.getTeacherAttendance(filters);
  ...
  async getTeacherAttendanceStats(req, res, next) {
    try {
      ...
      const stats =
        await this.attendanceService.getTeacherAttendanceStats(filters);
  ...
  async generateAttendanceReport(req, res, next) {
    try {
      ...
      const report =
        await this.attendanceService.generateAttendanceReport(filters);
  ...
  async getAttendanceDashboard(req, res, next) {
    try {
      ...
      const studentStats =
        await this.attendanceService.getStudentAttendanceStats(studentFilters);
      ...
      const teacherStats =
        await this.attendanceService.getTeacherAttendanceStats(teacherFilters);
