...
function defineTeacherAttendance(sequelize) {
  const TeacherAttendance = sequelize.define(
    "TeacherAttendance",
    {
      ...
      is_holiday: {
        ...
        defaultValue: false,
        comment:
          "Whether this date was a holiday (affects payroll calculations)",
      },
      is_substitute: {
        type: DataTypes.BOOLEAN,
        allowNull: false,
      ...
      additional_info: {
        ...
        allowNull: true,
        comment:
          "Additional attendance information (medical certificates, approvals, etc.)",
      },
      created_at: {
        type: DataTypes.DATE,
        allowNull: false,
  ...
  TeacherAttendance.calculateAttendancePercentage = async function (
    teacherId,
    startDate,
    endDate,
  ) {
  ...
  TeacherAttendance.calculateTotalHours = async function (
    teacherId,
    startDate,
    endDate,
  ) {
...
const teacherAttendanceValidationSchemas = {
  markAttendance: Joi.object({
    ...
    status: Joi.string()
      ...
      .messages({
        "any.only":
          "Status must be PRESENT, ABSENT, LATE, EXCUSED, SICK, LEAVE, or HALF_DAY",
    ...
    early_departure_minutes: Joi.number()
      .integer()
      .min(0)
      .max(1440)
      .allow(null)
