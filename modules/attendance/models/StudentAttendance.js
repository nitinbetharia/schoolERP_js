...
function defineStudentAttendance(sequelize) {
  const StudentAttendance = sequelize.define(
    "StudentAttendance",
    {
      ...
      additional_info: {
        ...
        allowNull: true,
        comment:
          "Additional attendance information (medical certificates, etc.)",
      },
      created_at: {
        type: DataTypes.DATE,
        allowNull: false,
  ...
  StudentAttendance.calculateAttendancePercentage = async function (
    studentId,
    startDate,
    endDate,
  ) {
...
const studentAttendanceValidationSchemas = {
  markAttendance: Joi.object({
    ...
    status: Joi.string()
      .valid("PRESENT", "ABSENT", "LATE", "EXCUSED", "SICK")
      .required()
      .messages({
  ...
  bulkMarkAttendance: Joi.object({
    ...
    attendance_records: Joi.array()
      ...
      .messages({
        ...
        "array.max":
          "Cannot mark attendance for more than 100 students at once",
      }),
  }),
