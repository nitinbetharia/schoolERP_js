      ...
      try {
         ...
         if (!student) {
            throw ErrorFactory.createError(
               'NOT_FOUND',
               'Student not found or inactive in this school',
            );
         }
   ...
   */
   async markBulkStudentAttendance(attendanceList, userId) {
      ...
      try {
         ...
         await this.logAttendanceActivity(
            'BULK_STUDENT_ATTENDANCE_MARKED',
            null,
            userId,
            {
   ...
   */
   async getStudentAttendanceStats(filters) {
      try {
        ...
        totalRecords > 0
           ? Math.round((presentRecords / totalRecords) * 100 * 100) / 100
           : 0;
   ...
   async markTeacherAttendance(attendanceData, userId) {
      try {
         ...
         if (!teacher) {
            throw ErrorFactory.createError(
               'NOT_FOUND',
               'Teacher not found or inactive in this school',
            );
         }
         ...
         const calculatedData = { ...attendanceData };
   ...
   async updateTeacherCheckOut(attendanceId, checkOutTime, userId) {
      try {
         ...
         if (!attendance.check_in_time) {
            throw ErrorFactory.createError(
               'BAD_REQUEST',
               'Cannot check out without check-in time',
            );
         }

         const totalHours = this.calculateHoursWorked(
         ...
         const earlyDepartureMinutes = this.calculateEarlyDepartureMinutes(
            checkOutTime,
            attendance.scheduled_end,
         );
