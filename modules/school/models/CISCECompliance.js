...
const defineCISCECompliance = (sequelize) => {
   const CISCECompliance = sequelize.define(
      'CISCECompliance',
      {
         ...
         affiliation_status: {
            type: DataTypes.ENUM('APPLIED', 'PROVISIONAL', 'PERMANENT', 'CONDITIONAL', 'SUSPENDED', 'WITHDRAWN'),
            allowNull: false,
         ...
         assessment_pattern: {
            type: DataTypes.ENUM('TRADITIONAL', 'INTERNAL_ASSESSMENT', 'CONTINUOUS_EVALUATION'),
            allowNull: false,
         ...
         nep_compatibility_assessment: {
            type: DataTypes.ENUM('NOT_ASSESSED', 'COMPATIBLE', 'PARTIALLY_COMPATIBLE', 'NOT_COMPATIBLE'),
            allowNull: false,
      ...
      },
      {
         ...
         hooks: {
            beforeValidate: (cisceCompliance) => {
               ...
               if (cisceCompliance.cisce_code) {
                  ...
                  if (!cisceCodeRegex.test(cisceCompliance.cisce_code)) {
                     throw new Error('CISCE code must be 4 uppercase letters followed by 4 digits');
                  }
               }

               // Validate examination years
               const currentYear = new Date().getFullYear();
               if (cisceCompliance.first_icse_exam_year && cisceCompliance.first_icse_exam_year > currentYear + 1) {
                  throw new Error('First ICSE exam year cannot be more than 1 year in the future');
               }
               if (cisceCompliance.first_isc_exam_year && cisceCompliance.first_isc_exam_year > currentYear + 1) {
                  throw new Error('First ISC exam year cannot be more than 1 year in the future');
               }
            },
            ...
            beforeSave: (cisceCompliance) => {
               ...
               else if (cisceCompliance.affiliation_status === 'PROVISIONAL') score += 15;
               else if (cisceCompliance.affiliation_status === 'CONDITIONAL') score += 10;

               // NEP compatibility (20 points)
               if (cisceCompliance.nep_compatibility_assessment === 'COMPATIBLE') score += 20;
               else if (cisceCompliance.nep_compatibility_assessment === 'PARTIALLY_COMPATIBLE') score += 12;
               ...
               const teacherCompliance = cisceCompliance.teacher_qualification_compliance || {};
               const infraCompliance = cisceCompliance.infrastructure_compliance || {};
