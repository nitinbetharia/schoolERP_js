   ...
   const InternationalBoardCompliance = sequelize.define(
      'InternationalBoardCompliance',
      {
         ...
         assessment_philosophy: {
            type: DataTypes.ENUM('FORMATIVE_FOCUS', 'SUMMATIVE_FOCUS', 'BALANCED', 'COMPETENCY_BASED'),
            allowNull: false,
         ...
         nep_integration_approach: {
            type: DataTypes.ENUM('NO_INTEGRATION', 'SELECTIVE', 'HYBRID', 'FULL_INTEGRATION'),
            allowNull: false,
         ...
         compliance_rating: {
            type: DataTypes.ENUM('EXEMPLARY', 'PROFICIENT', 'DEVELOPING', 'BEGINNING', 'NOT_MEETING'),
            allowNull: false,
      ...
      },
      {
         ...
         hooks: {
            beforeValidate: (intlBoardCompliance) => {
               ...
               if (intlBoardCompliance.authorization_expiry && intlBoardCompliance.authorization_date) {
                  const authDate = new Date(intlBoardCompliance.authorization_date);
                  const expiryDate = new Date(intlBoardCompliance.authorization_expiry);
                  if (expiryDate <= authDate) {
                     throw new Error('Authorization expiry date must be after authorization date');
                  }
               }
            },
            ...
            beforeSave: (intlBoardCompliance) => {
               ...
               const teacherCompliance = intlBoardCompliance.teacher_training_compliance || {};
               ...
               const facilitiesCompliance = intlBoardCompliance.specialized_facilities_compliance || {};
               ...
               const reportingCompliance = intlBoardCompliance.annual_reporting_requirements || {};
               ...
               else if (score >= 75) intlBoardCompliance.compliance_rating = 'PROFICIENT';
               else if (score >= 60) intlBoardCompliance.compliance_rating = 'DEVELOPING';
               else if (score >= 40) intlBoardCompliance.compliance_rating = 'BEGINNING';
