      ...
      },
      {
         ...
         hooks: {
            ...
            beforeSave: (cbseCompliance) => {
               ...
               else if (cbseCompliance.affiliation_status === 'PROVISIONAL')
               {score += 10;}

               // NEP adoption (20 points)
               if (cbseCompliance.nep_adoption_status === 'FULL') {score += 20;}
               else if (cbseCompliance.nep_adoption_status === 'PARTIAL')
               {score += 10;}
               ...
               if (cbseCompliance.assessment_framework === 'NEP_2020_COMPATIBLE')
               {score += 20;}
               else if (cbseCompliance.assessment_framework === 'CBSE_ASSESSMENT')
               {score += 15;}

               // Submissions compliance (20 points)
               const submissions = cbseCompliance.mandatory_submissions || {};
               const completedSubmissions = Object.values(submissions).filter(
                  (s) => s === 'COMPLETED',
               ).length;
