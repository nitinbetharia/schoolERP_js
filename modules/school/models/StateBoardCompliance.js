...
const defineStateBoardCompliance = (sequelize) => {
   const StateBoardCompliance = sequelize.define(
      'StateBoardCompliance',
      {
         ...
         registration_status: {
            type: DataTypes.ENUM('APPLIED', 'PROVISIONAL', 'PERMANENT', 'TEMPORARY', 'SUSPENDED', 'CANCELLED'),
            allowNull: false,
         ...
         curriculum_framework: {
            type: DataTypes.ENUM('STATE_CURRICULUM', 'NCF_2005', 'NEP_2020_ALIGNED', 'HYBRID'),
            allowNull: false,
            defaultValue: 'STATE_CURRICULUM',
         },
         assessment_pattern: {
            type: DataTypes.ENUM('ANNUAL', 'SEMESTER', 'CONTINUOUS_COMPREHENSIVE', 'COMPETENCY_BASED'),
            allowNull: false,
            defaultValue: 'ANNUAL',
         },
         grading_system: {
            type: DataTypes.ENUM('PERCENTAGE', 'GRADE_POINTS', 'LETTER_GRADES', 'DESCRIPTIVE'),
            allowNull: false,
         ...
         nep_implementation_status: {
            type: DataTypes.ENUM('NOT_STARTED', 'PLANNING', 'PILOT', 'PARTIAL', 'FULL'),
            allowNull: false,
      ...
      },
      {
         ...
         hooks: {
            beforeValidate: (stateBoardCompliance) => {
               ...
               if (stateBoardCompliance.renewal_due_date) {
                  ...
                  if (renewalDate <= today) {
                     console.warn('Renewal due date should be in the future for active compliance');
                  }
               }
            },
            ...
            beforeSave: (stateBoardCompliance) => {
               ...
               if (stateBoardCompliance.registration_status === 'PERMANENT') score += 25;
               else if (stateBoardCompliance.registration_status === 'PROVISIONAL') score += 18;
               else if (stateBoardCompliance.registration_status === 'TEMPORARY') score += 12;
               ...
               const teacherCompliance = stateBoardCompliance.teacher_eligibility_compliance || {};
               if (Object.keys(teacherCompliance).length > 0) score += 15;

               // Infrastructure compliance (10 points)
               const infraCompliance = stateBoardCompliance.infrastructure_norms_compliance || {};
               if (Object.keys(infraCompliance).length > 0) score += 10;

               // Reporting compliance (10 points)
               const annualReport = stateBoardCompliance.annual_report_submission || {};
               const statisticalData = stateBoardCompliance.statistical_data_submission || {};
               ...
               else if (score >= 60) stateBoardCompliance.compliance_status = 'SATISFACTORY';
               else if (score >= 40) stateBoardCompliance.compliance_status = 'NEEDS_IMPROVEMENT';
