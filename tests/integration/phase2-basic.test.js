...
describe('Phase 2: Basic Frontend & API Tests', () => {
   ...
   describe('Server Health Tests', () => {
      test('Server is running and responds', async () => {
         const response = await request('http://localhost:3000').get('/api/v1/status').expect(200);
      ...
      test('Health check endpoint works', async () => {
         const response = await request('http://localhost:3000').get('/api/v1/admin/system/health').expect(200);
   ...
   describe('Static Assets Tests', () => {
      test('CSS files are accessible', async () => {
         const response = await request('http://localhost:3000').get('/css/bootstrap.min.css').expect(200);

         expect(response.headers['content-type']).toContain('text/css');
      });

      test('JavaScript files are accessible', async () => {
         const response = await request('http://localhost:3000').get('/js/bootstrap.bundle.min.js').expect(200);
   ...
   describe('API Endpoints Basic Tests', () => {
      test('User permissions endpoint exists', async () => {
         ...
         const response = await request('http://localhost:3000').get('/api/admin/user-permissions');
      ...
      test('User statistics endpoint exists', async () => {
         // This should require authentication, so we expect 401
         const response = await request('http://localhost:3000').get('/api/admin/user-statistics');
      ...
      test('User creation endpoint exists', async () => {
         // This should require authentication, so we expect 401
         const response = await request('http://localhost:3000').post('/api/admin/users').send({});
   ...
   describe('View Templates Tests', () => {
      test('User registration template exists', async () => {
         ...
         if (templateExists) {
            ...
            expect(templateContent).toContain('container-fluid');
            expect(templateContent).toContain('createUserModal');
            expect(templateContent).toContain('User Registration');
         }
      });
