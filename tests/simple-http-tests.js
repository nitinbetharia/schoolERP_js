...
class SimpleHttpTester {
   ...
   async testSystemStatus() {
      ...
      if (result.success && result.statusCode === 200) {
         ...
         this.log(`❌ System status test failed: ${result.error || result.statusCode}`, 'result');
      }
   ...
   async testSystemHealth() {
      ...
      if (result.success) {
         this.log(`✅ System health test completed (Status: ${result.statusCode})`, 'result');
   ...
   async testSubdomainDetection(subdomain) {
      ...
      if (result.success) {
         this.log(`✅ Subdomain ${subdomain} test completed (Status: ${result.statusCode})`, 'result');
         if (result.data && typeof result.data === 'object') {
            this.log(`Response: ${JSON.stringify(result.data, null, 2)}`);
         }
      } else {
         this.log(`❌ Subdomain ${subdomain} test failed: ${result.error}`, 'result');
      }
   ...
   async testSystemAdminAuth() {
      ...
      if (result.success && result.statusCode === 200) {
         ...
         this.log('⚠️  System admin authentication failed (401 - check credentials)', 'result');
      } else {
         this.log(`❌ System admin authentication test failed: ${result.error || result.statusCode}`, 'result');
      }
   ...
   async runAllTests() {
      ...
      const tests = [
         ...
         { name: 'Trust001 Subdomain', fn: () => this.testSubdomainDetection('trust001') },
         { name: 'Demo Subdomain', fn: () => this.testSubdomainDetection('demo') },
         { name: 'Invalid Subdomain', fn: () => this.testSubdomainDetection('nonexistent') },
      ...
      for (const test of tests) {
         ...
         try {
            ...
            this.log(`❌ Test ${test.name} threw error: ${error.message}`, 'result');
         }
