...
describe("🌱 SchoolERP Data Seeding Suite", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      "🚀 Initializing SchoolERP Server for Comprehensive Data Seeding...",
    );
  ...
  describe("Phase 1: Foundation Layer (System Routes)", () => {
    test("Should create primary test trust with complete setup", async () => {
      ...
      console.log(
        `✅ Primary Trust created: ID=${trustId}, Code=${trustData.trust_code}`,
      );
    });
    ...
    test("Should manually initialize tenant models for primary trust", async () => {
      ...
      try {
        ...
        console.log(
          `✅ Tenant database created and models initialized for: ${trustCode}`,
        );
        console.log(
          `   Available models: ${Object.keys(tenantModels).join(", ")}`,
        );
      } catch (error) {
    ...
    test("Should create secondary test trust for multi-tenant scenarios", async () => {
      ...
      console.log(
        `✅ Secondary Trust created: ID=${trustId}, Code=${trustData.trust_code}`,
      );
      ...
      try {
        ...
        console.log(
          `✅ Secondary trust tenant database and models initialized: ${trustData.trust_code}`,
        );
      } catch (error) {
      ...
      try {
        ...
        for (const trust of SEED_DATA.trusts) {
          try {
            ...
            console.log(
              `✅ School created: ${school.name} (ID: ${school.id}) for trust ${trust.trust_code}`,
            );
          } catch (error) {
            console.log(
              `⚠️ School creation failed for trust ${trust.trust_code}: ${error.message}`,
            );
          }
        }
      ...
      try {
        ...
        for (const school of SEED_DATA.schools) {
          try {
            ...
            const academicYear =
              await tenantModels.AcademicYear.create(academicYearData);
            ...
            console.log(
              `⚠️ Academic year creation failed for ${school.name}: ${error.message}`,
            );
          }
        }
        ...
        console.log(
          `📊 Total academic years created: ${SEED_DATA.academicYears.length}`,
        );
      } catch (error) {
        console.log(
          `⚠️ Academic year creation process failed: ${error.message}`,
        );
        expect(error).toBeDefined();
      }
    });
      ...
      try {
        ...
        for (const school of SEED_DATA.schools) {
          try {
            ...
            console.log(
              `✅ Users created for: ${school.name} (Principal + Teacher)`,
            );
          } catch (error) {
            console.log(
              `⚠️ User creation failed for ${school.name}: ${error.message}`,
            );
          }
        }
    ...
    test("Should create classes and sections", async () => {
      ...
      try {
        ...
        for (const school of SEED_DATA.schools) {
          try {
            const tenantModels = await getTenantModels(school.trust_code);
            const academicYear = SEED_DATA.academicYears.find(
              (ay) => ay.school_id === school.id,
            );

            if (!academicYear) {
              console.log(
                `⚠️ No academic year found for school ${school.name}`,
              );
              continue;
            }
            ...
            console.log(
              `✅ Classes created for: ${school.name} (Grades 1-5, Sections A-B)`,
            );
          } catch (error) {
            console.log(
              `⚠️ Class creation failed for ${school.name}: ${error.message}`,
            );
          }
        }
      ...
      try {
        ...
        for (const school of SEED_DATA.schools) {
          try {
            ...
            console.log(
              `✅ Students created for: ${school.name} (3 sample students)`,
            );
          } catch (error) {
            console.log(
              `⚠️ Student creation failed for ${school.name}: ${error.message}`,
            );
          }
        }
  ...
  describe("Phase 5: Final Verification & Summary", () => {
    test("Should verify all seeded data and provide comprehensive summary", async () => {
      ...
      SEED_DATA.trusts.forEach((trust, idx) => {
        console.log(
          `   ${idx + 1}. ${trust.trust_name} (${trust.trust_code}) - Status: ${trust.status}`,
        );
      });
      ...
      SEED_DATA.schools.forEach((school, idx) => {
        console.log(
          `   ${idx + 1}. ${school.name} (${school.code}) - Trust: ${school.trust_code}`,
        );
      });
      ...
      SEED_DATA.academicYears.forEach((ay, idx) => {
        console.log(
          `   ${idx + 1}. ${ay.year_label} - School: ${ay.school_name} (${ay.trust_code})`,
        );
      });
      ...
      SEED_DATA.users.forEach((user, idx) => {
        console.log(
          `   ${idx + 1}. ${user.full_name} (${user.role}) - School: ${user.school_name}`,
        );
      });
      ...
      // Verification
      const hasBasicData =
        SEED_DATA.trusts.length >= 2 && SEED_DATA.schools.length >= 0;
