...
describe.skip('Subdomain-based Multi-tenant Authentication', () => {
   ...
   describe('System Admin Authentication (No Tenant)', function () {
      ...
      it('should authenticate system admin successfully', async function () {
         ...
         const response = await axios.post(`${baseURL}/api/v1/admin/system/auth/login`, loginData);
      ...
      it('should access system health endpoint', async function () {
         const response = await axios.get(`${baseURL}/api/v1/admin/system/health`, {
            headers: {
   ...
   describe('Trust001 Subdomain Tests', function () {
      ...
      it('should create a user on trust001 subdomain', async function () {
         ...
         try {
            ...
            if (response.status === 201) {
               expect(response.data).to.have.property('success', true);
               expect(response.data.data.user).to.have.property('username', 'principal001');
            } else {
               console.log('User creation response:', response.status, response.data);
      ...
      it('should authenticate user on trust001 subdomain', async function () {
         ...
         try {
            const response = await axios.post(`${baseURL}/api/v1/users/auth/login`, loginData, {
               headers: {
                  ...trust001Headers,
                  'Content-Type': 'application/json',
               },
   ...
   describe('Demo Subdomain Tests', function () {
      ...
      it('should create a user on demo subdomain', async function () {
         ...
         try {
            ...
            if (response.status === 201) {
               expect(response.data).to.have.property('success', true);
               expect(response.data.data.user).to.have.property('username', 'demo_principal');
            }
         } catch (error) {
   ...
   describe('Localhost Fallback Tests', function () {
      it('should use default tenant for localhost requests', async function () {
         try {
            const response = await axios.get(`${baseURL}/api/v1/users`, { validateStatus: () => true });
   ...
   describe('Cross-Tenant Isolation Tests', function () {
      ...
      it('should reject login with wrong tenant credentials', async function () {
         ...
         try {
            const response = await axios.post(`${baseURL}/api/v1/users/auth/login`, demoCredentials, {
               headers: {
                  ...trust001Headers,
                  'Content-Type': 'application/json',
               },
            ...
            console.log('Wrong tenant credentials test error (acceptable):', error.message);
         }
      });
   });
   ...
   describe('Error Handling Tests', function () {
      it('should handle invalid subdomain gracefully', async function () {
         ...
         try {
            ...
            console.log('Invalid subdomain test error (acceptable):', error.message);
         }
      });
