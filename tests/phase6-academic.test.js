...
describe("Phase 6 - Academic Management System", () => {
  ...
  beforeAll(async () => {
    ...
    const sectionResponse = await request(app)
      .post(
        `/api/v1/trust/${trustId}/schools/${schoolId}/classes/${classId}/sections`,
      )
      .set("Cookie", authCookie)
      .send(sectionData);
  ...
  describe("Grade Management", () => {
    ...
    test("Should get student grades", async () => {
      const response = await request(app)
        .get(
          `/api/v1/trust/${trustId}/schools/${schoolId}/students/${studentId}/grades`,
        )
        .set("Cookie", authCookie);
  ...
  describe("Assessment Analytics", () => {
    test("Should get exam statistics", async () => {
      const response = await request(app)
        .get(
          `/api/v1/trust/${trustId}/schools/${schoolId}/exams/${examId}/statistics`,
        )
        .set("Cookie", authCookie);
  ...
  describe("Report Card Generation", () => {
    ...
    test("Should generate bulk report cards for class", async () => {
      const response = await request(app)
        .post(
          `/api/v1/trust/${trustId}/schools/${schoolId}/classes/${classId}/bulk-report-cards`,
        )
        .set("Cookie", authCookie)
        .send({
          academic_year_id: academicYearId,
  ...
  describe("Transcript Management", () => {
    ...
    test("Should get transcript history for student", async () => {
      const response = await request(app)
        .get(
          `/api/v1/trust/${trustId}/schools/${schoolId}/students/${studentId}/transcripts`,
        )
        .set("Cookie", authCookie);
  ...
  describe("Academic Calendar", () => {
    ...
    test("Should get academic calendar events", async () => {
      const response = await request(app)
        .get(
          `/api/v1/trust/${trustId}/schools/${schoolId}/calendar?academic_year_id=${academicYearId}`,
        )
        .set("Cookie", authCookie);
  ...
  describe("Grade Book Management", () => {
    test("Should create grade book entry", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data).toHaveProperty("id");
      expect(response.body.data.assignment_name).toBe(
        gradeBookData.assignment_name,
      );
    });
    ...
    test("Should get grade book for class", async () => {
      const response = await request(app)
        .get(
          `/api/v1/trust/${trustId}/schools/${schoolId}/gradebook?class_id=${classId}&subject_id=${subjectId}`,
        )
        .set("Cookie", authCookie);
