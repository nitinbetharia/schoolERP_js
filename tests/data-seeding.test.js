...
describe('Data Seeding - API Based Setup', () => {
   ...
   describe('Sequence 2: Trust Setup', () => {
      ...
      test('Should create secondary test trust for multi-tenant testing', async () => {
         ...
         console.log(`✅ Secondary trust created and setup completed with ID: ${trustId}`);
      });
   });
   ...
   describe('Sequence 3: School Management Setup', () => {
      test('Should check if school routes are enabled', async () => {
         ...
         if (response.status === 400) {
            console.log('✅ Trust-scoped school routes are working (400 expected for validation)');
         } else if (response.status === 404) {
            console.log('⚠️ School routes appear to be disabled - will use setup routes instead');
         } else {
      ...
      test('Should create primary school via available endpoints', async () => {
         ...
         try {
            ...
            if (response.status === 404) {
               ...
               response = await request(app).post('/api/v1/schools').set('Cookie', authCookie).send(schoolData);
            }
            ...
            console.log('⚠️ School creation endpoint not found, will log available routes');
         }

         console.log(`🔍 School creation attempt status: ${response ? response.status : 'No response'}`);

         if (response && response.status === 201) {
            seededData.schools.push(response.body.data);
            console.log(`✅ Primary school created with ID: ${response.body.data.id}`);
         } else {
            console.log('⚠️ School creation needs investigation - will use mock data for now');
      ...
      test('Should create academic years for each school', async () => {
         ...
         for (const school of seededData.schools) {
            ...
            try {
               ...
               response = await request(app)
                  .post(`/api/v1/trust/${trustId}/schools/${school.id}/academic-years`)
                  .set('Cookie', authCookie)
            ...
            if (response && response.status === 201) {
               ...
               console.log(`⚠️ Academic year creation needs investigation for school ${school.name}`);
               // Add mock data
      ...
      test('Should discover all available API endpoints', async () => {
         ...
         for (const endpoint of endpoints) {
            try {
               ...
               console.log(`⚠️ ${endpoint.method} ${endpoint.path} - Error occurred`);
            }
         }
