# GitHub Copilot Instructions - School ERP System (UPDATED)
...
function createStudentService() {
  ...
  async function createStudent(studentData) {
    try {
      ...
      const student = await db.executeWithRetry(
...
function createPerformanceMonitor() {
  ...
  function middleware() {
    return function(req, res, next) {
      ...
      res.on('finish', function() {
        ...
        if (duration > 500) { // 500ms instead of 1000ms
  ...
  function recordMetric(req, res, duration) {
    ...
    const sum = metrics.responseTimes.reduce(function(a, b) { return a + b; }, 0);
...
2. ✅ **Check existing database connections** in `modules/data/`:
   ...
   - Multi-tenant database naming (school_erp_trust_{code})
