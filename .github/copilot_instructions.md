...
function createStudentService() {
  ...
  async function createStudent(studentData) {
    try {
      ...
      const student = await db.executeWithRetry("INSERT INTO students SET ?", [
        validated,
      ]);
...
function createDatabaseService() {
  ...
  async function executeWithRetry(query, params, options) {
    ...
    const retryDelay =
      options && options.retryDelay ? options.retryDelay : 1000;
  ...
  async function healthCheck() {
    try {
      ...
      return {
        ...
        poolUsed:
          pool.pool.allConnections.length - pool.pool.freeConnections.length,
      };
    } catch (error) {
      return {
        status: "unhealthy",
    ...
    if (!connections.has(trustCode)) {
      ...
      const totalConnections = Array.from(connections.values()).reduce(
        function (sum, pool) {
          return (
            sum +
            (pool.pool.allConnections ? pool.pool.allConnections.length : 0)
          );
        },
        0,
      );
          ...
          if (now - stats.lastActivity.getTime() > idleTimeout) {
            ...
            if (pool) {
              pool
                .end()
                .then(function () {
                  logger.info("Closed idle tenant connection", {
                    trustCode: trustCode,
                  });
  ...
  async function healthCheckAllTenants() {
    ...
    for (const entry of connections.entries()) {
      ...
      try {
        ...
        const poolSize = pool.pool.allConnections
        ...
        results[trustCode] = {
          ...
          poolUsed:
            poolSize -
            (pool.pool.freeConnections ? pool.pool.freeConnections.length : 0),
        };
      } catch (error) {
        results[trustCode] = {
          status: "unhealthy",
        ...
        runAllChecks()
          .then(function (health) {
            ...
            if (
              ...
              health.checks.memory.data.status === "warning" &&
              global.gc
            ) {
              global.gc();
              logger.info(
