...
async function up(queryInterface, _Sequelize) {
   ...
   try {
      ...
      await queryInterface.createTable('system_users', {
         ...
         role: {
            type: DataTypes.ENUM(
               'SYSTEM_ADMIN', 'TRUST_ADMIN', 'SCHOOL_ADMIN', 
               'PRINCIPAL', 'TEACHER', 'STAFF', 'STUDENT'
            ),
            allowNull: false
         },
         is_active: {
      ...
      return {
         ...
         indexes_created: [
            'idx_trusts_code', 'idx_trusts_subdomain', 'idx_trusts_status',
            'idx_system_users_trust_role', 'idx_system_users_username', 'idx_system_users_email',
            'idx_audit_user_time', 'idx_audit_trust_time'
         ]
      };
