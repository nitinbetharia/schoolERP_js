...
class EmailService {
   ...
   async sendPasswordResetEmail(user, resetToken, tenant = null) {
      try {
         ...
         const emailData = await templateManager.generateEmail(
            templateManager.EMAIL_TYPES.PASSWORD_RESET,
            { user, resetToken, tenant }
         );
   ...
   async sendPasswordResetSuccessEmail(user, tenant = null, ipAddress = null) {
      try {
         ...
         // Generate email content
         const emailData = await templateManager.generateEmail(
            templateManager.EMAIL_TYPES.PASSWORD_RESET_SUCCESS,
   ...
   async sendWelcomeEmail(user, tempPassword = null, tenant = null, createdBy = null) {
      try {
         ...
         // Generate email content
         const emailData = await templateManager.generateEmail(
            templateManager.EMAIL_TYPES.WELCOME_USER,
            { user, tempPassword, tenant, createdBy }
         );
   ...
   async sendNotificationEmail(user, subject, message, options = {}) {
      try {
         ...
         // Generate email content
         const emailData = await templateManager.generateEmail(
            templateManager.EMAIL_TYPES.NOTIFICATION,
            emailOptions
         );
